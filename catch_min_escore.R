...
scrape.data <- function(input_list){
  ...
  for (i in seq_along(input_list)) {
    working_data <- read.table(file = input_list[[i]], sep='\t', col.names=c("*","gene_name","KO","thrshld","score","Evalue","KO_definition"))
    print(working_data)
...
write.csv(scraped_data %>% group_by(genome,KO) %>% summarize(min=min(Evalue)),"./results/caught_min_escore_genes.csv")
